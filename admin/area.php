...
// --- SINKRONISASI OTOMATIS KAPASITAS ---
// Setiap kali halaman dibuka, hitung ulang kapasitas_tersedia berdasarkan
// jumlah transaksi aktif di setiap area. Ini menjaga data tetap akurat.
...
while ($row = mysqli_fetch_assoc($areas_sync)) {
    ...
    // Hitung berapa kendaraan yang aktif (status='masuk') di area ini
    ...
    // Update kapasitas tersedia ke database
...
if (isset($_POST['update'])) {
    ...
    // Ambil data lama untuk menghitung ulang kapasitas tersedia
    ...
    // Pastikan kapasitas tersedia tidak negatif
    if ($tersedia_baru < 0)
        $tersedia_baru = 0;
...
// Ambil semua data area untuk ditampilkan sebagai kartu
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Form Tambah / Edit Area -->
        <div class="card-form">
            <!-- Judul form berubah sesuai mode (tambah atau edit) -->
            <h3>
                <i class="fas <?= $edit ? 'fa-edit' : 'fa-plus-circle' ?>" style="color: var(--orange-main);"></i>
                <?= $edit ? "Perbarui Informasi Area" : "Tambah Area Strategis" ?>
            </h3>
            <form method="post" class="form-grid">
                <!-- Hidden ID hanya terisi saat mode edit -->
                <input type="hidden" name="id_area" value="<?= $edit['id_area'] ?? '' ?>">
                <input name="nama_area" placeholder="Contoh: Gedung A, Basement 1" required
                    value="<?= $edit['nama_area'] ?? '' ?>">
                <input type="number" name="kapasitas_total" placeholder="Kapasitas Slot" required
                    value="<?= $edit['kapasitas_total'] ?? '' ?>">
                <!-- Nama tombol submit berubah sesuai mode -->
